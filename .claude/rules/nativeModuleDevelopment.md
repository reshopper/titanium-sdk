---
description: Titanium SDK Native Module Development Rules
...
### Creating Views via Factory Methods (`createView`)

For a user to create your custom view in JavaScript via a factory method (e.g., `Ti.UI.createMyView()`), Titanium handles this differently on iOS and Android. The goal is the same: to generate a `create<ViewName>` method on a module (typically `UIModule`), but the implementation varies.

*   **iOS (Objective-C & Swift):** The creation is **automatic**. As soon as you subclass `TiViewProxy`, Titanium will automatically generate the corresponding `create...` method. No further configuration is required.

*   **Android (Java):** You must **explicitly** request that the method be generated. This is done by adding `creatableInModule` to the `@Kroll.proxy` annotation on your `ViewProxy` class.

    ```java
    // This line instructs Titanium to generate "createMyView" on UIModule
...
Although the underlying mechanism is different, the result for the JavaScript developer is the same across platforms.
